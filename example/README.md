...
5. check API {host}/test/users, you should see a list of users now.
6. swagger access: https://stackoverflow.com/questions/37855489/export-swagger-api-definition-from-api-gateway-via-http-request

### development
1. `npm test` for testing
2. `npm start` for starting a local express server
3. `sam local start-api --template cloudformation.yaml` for local integration test, inside a mocked apigateway enviroment
